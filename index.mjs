...
/**
 * выводит данные файла. если задан поиск, то ищет совпадения в файле и выводит строки с совпадениями
 * @param {string} path
 */
const getFileData = (path) => {
    ...
    if (options.search) {
        ...
        if (matches.length) {
            log(
                colors.green(`${matches.length} matches in the following lines`)
            );
...
/**
 * возвращает объект с именем файла/папки и абсолютным путем до файла/папки
 * @param {string} path
...
/**
 * читает путь. если это файл, выводит данные, иначе выводит данные папки
 * @param {string} path
 */
const reader = (path) => {
    ...
    const listFilesWithPath = listFiles.map((filename) =>
        getPathway(path, filename)
    );
